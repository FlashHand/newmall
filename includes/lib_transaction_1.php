...
function get_user_orders_1($user_id, $num = 10, $start = 0,$where='')
{
    ...
    while ($row = $GLOBALS['db']->fetchRow($res))
    {
        ...
        {
            $row['handler'] = "<a style=\"display:inline-block;background:#F60;color:#fff;padding:3px 5px ;margin:3px 0px;\" href=\"user.php?act=cancel_order&order_id=" .$row['order_id']. "\" onclick=\"if (!confirm('".$GLOBALS['_LANG']['confirm_cancel']."')) return false;\">".$GLOBALS['_LANG']['cancel']."</a>";
        }
        else if ($row['order_status'] == OS_SPLITED)
        {
            ...
            if ($row['shipping_status'] == SS_SHIPPED)
            {
                ...
                @$row['handler'] = "<a href=\"user.php?act=affirm_received&order_id=" .$row['order_id']. "\" onclick=\"if (!confirm('".$back_info.$GLOBALS['_LANG']['confirm_received']."')) return false;\" style='display:inline-block;background:#F60;color:#fff;padding:3px 5px ;margin:3px 0px;'>".$GLOBALS['_LANG']['received']."</a>";
            }
